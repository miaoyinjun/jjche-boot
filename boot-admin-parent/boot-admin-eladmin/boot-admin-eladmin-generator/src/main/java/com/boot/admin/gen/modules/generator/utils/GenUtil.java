package com.boot.admin.gen.modules.generator.utils;
...
public class GenUtil {
    ...
    public static String download(List<ColumnInfoDO> columns, GenConfigDO genConfig) throws IOException {
        // 拼接的路径：/tmpboot-admin-eladmin-gen-temp/，这个路径在Linux下需要root用户才有权限创建,非root用户会权限错误而失败，更改为： /tmp/boot-admin-eladmin-gen-temp/
        // String tempPath =SYS_TEM_DIR + "boot-admin-eladmin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        String tempPath = SYS_TEM_DIR + "boot-admin-eladmin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        Map<String, Object> genMap = getGenMap(columns, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
        // 生成后端代码
...
//        String userDir = System.getProperty("user.dir");
    ...
    private static Map<String, Object> getGenMap(List<ColumnInfoDO> columnInfos, GenConfigDO genConfig) {
        ...
        // 大写开头的类名
        String className = com.boot.admin.common.util.StrUtil.toCapitalizeCamelCase(genConfig.getTableName());
        // 小写开头的类名
        String changeClassName = com.boot.admin.common.util.StrUtil.toCamelCase(genConfig.getTableName());
        // 判断是否去除表前缀
        if (com.boot.admin.common.util.StrUtil.isNotEmpty(genConfig.getPrefix())) {
            className = com.boot.admin.common.util.StrUtil.toCapitalizeCamelCase(StrUtil.removePrefix(genConfig.getTableName(), genConfig.getPrefix()));
            changeClassName = com.boot.admin.common.util.StrUtil.toCamelCase(StrUtil.removePrefix(genConfig.getTableName(), genConfig.getPrefix()));
        }
        String controllerBaseUrl = com.boot.admin.common.util.StrUtil.pluralizeWord(genConfig.getTableName());
        // 保存类名
        ...
        for (ColumnInfoDO column : columnInfos) {
            ...
            // 小写开头的字段名
            String changeColumnName = com.boot.admin.common.util.StrUtil.toCamelCase(columnName);
            // 大写开头的字段名
            String capitalColumnName = com.boot.admin.common.util.StrUtil.toCapitalizeCamelCase(column.getColumnName());
            ...
            // 主键存在字典
            if (com.boot.admin.common.util.StrUtil.isNotBlank(column.getDictName())) {
            ...
            // 表单组件类型
            listMap.put("formType", com.boot.admin.common.util.StrUtil.isNotBlank(column.getFormType()) ? column.getFormType() : "Input");
            // 小写开头的字段名称
            ...
            if (!com.boot.admin.common.util.StrUtil.isBlank(column.getQueryType())) {
                // 查询类型
