  ...
  <#if betweens??>
    <#list betweens as column>
      <#if column.queryType = 'BetWeen'>
        <date-range-picker
          v-model="query.${column.changeColumnName}"
          start-placeholder="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>开始"
          end-placeholder="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>结束"
          class="date-item"
        />
      </#if>
    </#list>
    ...
    <#if columns??>
      ...
      >
        ...
        <#if columns??>
            <#list columns as column>
            <#if column.columnShow && pkChangeColName != column.changeColumnName>
          <#if (column.dictName)?? && (column.dictName)!="">
        ...
        <el-table-column prop="${column.changeColumnName}" label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>">
          <template slot-scope="scope">
            <span>{{ scope.row.${column.changeColumnName} }}</span>
          </template>
        </el-table-column>
                </#if>
            </#if>
