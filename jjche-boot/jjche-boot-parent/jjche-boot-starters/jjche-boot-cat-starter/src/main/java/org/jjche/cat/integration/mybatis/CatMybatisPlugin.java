package org.jjche.cat.integration.mybatis;
...
public class CatMybatisPlugin implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        Transaction t = Cat.newTransaction(CatConstant.TYPE_SQL, methodName);

        String sql = this.getSql(invocation, mappedStatement);
        SqlCommandType sqlCommandType = mappedStatement.getSqlCommandType();
        Cat.logEvent(CatConstant.TYPE_SQL_METHOD, sqlCommandType.name().toLowerCase(), Message.SUCCESS, sql);

        String url = this.getSQLDatabaseUrlByStatement(mappedStatement);
        Cat.logEvent(CatConstant.TYPE_SQL_DB, url);
