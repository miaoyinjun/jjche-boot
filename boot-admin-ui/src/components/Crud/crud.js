...
function CRUD(options) {
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          let meta = data.meta
          if (meta != null) {
            meta = meta.filter(item => item.isAccessible === true)
            crud.meta = meta
          }
    ...
    dleChangePage(size) {
      if (crud.data.length === size && crud.page.pageIndex !== 1) {
        crud.page.pageIndex -= 1
      }
    },
    // 选择改变
